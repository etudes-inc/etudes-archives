/**********************************************************************************
 * $URL: https://source.etudes.org/svn/apps/archives/trunk/archives-plugin/plugin/src/java/org/etudes/archives/plugin/PurgeBlogHandler.java $
 * $Id: PurgeBlogHandler.java 2823 2012-04-03 20:57:39Z ggolden $
 ***********************************************************************************
 *
...
/**
 * Archives PurgeHandler for Blog
 */
public class PurgeBlogHandler implements PurgeHandler
{
	/** Our log. */
	private static Log M_log = LogFactory.getLog(PurgeBlogHandler.class);

	/** The application Id. */
	protected final static String applicationId = "sakai.blog";
	...
	public void purge(String siteId)
	{
		...
		delete("DELETE BLOGGER_IMAGE FROM BLOGGER_IMAGE                                                              "
				+ " INNER JOIN BLOGGER_POST ON BLOGGER_IMAGE.POST_ID = BLOGGER_POST.POST_ID                          "
				+ " WHERE BLOGGER_POST.SITE_ID = ?", fields);
		delete("DELETE BLOGGER_FILE FROM BLOGGER_FILE                                                                "
				+ " INNER JOIN BLOGGER_POST ON BLOGGER_FILE.POST_ID = BLOGGER_POST.POST_ID                           "
				+ " WHERE BLOGGER_POST.SITE_ID = ?", fields);
		delete("DELETE FROM BLOGGER_POST WHERE SITE_ID = ?", fields);
	}
